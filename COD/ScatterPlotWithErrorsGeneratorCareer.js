function generateScatterPlotWithErrorsCareer(
  containerWidth,
  containerHeight,
  margin,
  data,
  ...
  svg.append("rect").attr("fill", "#fff").attr("rx", 8)

  // Define scales with padding
  const x = d3
    .scaleBand()
    .domain(sortedData.map((d) => d.stat))
    .range([0, width])
    .padding(0.4)

  const y = d3
    .scaleLinear()
    .domain([0, d3.max(sortedData, (d) => d.max) * 1.1].reverse()) // Add 10% padding at the top
    .nice()
    .range([height, 0])
